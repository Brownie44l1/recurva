...
- A running Recurva instance (`docker compose up`) or access to `https://api.recurva.com`
- `curl` (or any HTTP client)

## Step 1: Register a Tenant

```bash
curl -X POST https://api.recurva.com/v1/tenants/register \
...
Save the `apiKey` from the response — you'll use it for all subsequent requests.
...
```bash
curl -X POST https://api.recurva.com/v1/plans \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://api.recurva.com/v1/customers \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer rcv_live_a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4" \
  -d '{
...
```bash
curl -X POST https://api.recurva.com/v1/subscriptions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer rcv_live_a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4" \
  -d '{
...
```bash
curl -X GET https://api.recurva.com/v1/subscriptions/<subscription-id> \
  -H "Authorization: Bearer rcv_live_a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4"
```
...
```bash
curl -X POST https://api.recurva.com/v1/webhooks/endpoints \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer rcv_live_a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4" \
  -d '{
